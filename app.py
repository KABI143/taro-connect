...
def upload():
    ...
    if "MotorRunningStatus" in df.columns:
        for i in range(1, len(df)):
            prev = df["MotorRunningStatus"].iloc[i - 1]
            curr = df["MotorRunningStatus"].iloc[i]
            if bool(prev) is False and bool(curr) is True:
                start_count += 1
    ...
    if "PackCount" in df.columns:
        pack_data = df["PackCount"].fillna(0)
        prev = pack_data.iloc[0]
        for cur in pack_data.iloc[1:]:
            if cur >= prev:
                pack_count_total += cur - prev
            prev = cur
    ...
    if "Total Running Time" in df.columns:
        rt   = df["Total Running Time"].fillna(0)
        prev = rt.iloc[0]
        for cur in rt.iloc[1:]:
            if cur >= prev:
                total_runtime += cur - prev
            prev = cur
